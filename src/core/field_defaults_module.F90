MODULE FIELD_DEFAULTS_MODULE
  ...
  INTEGER(KIND=JPIM), PROTECTED  :: FIELD_NQUEUES = 1
...
CONTAINS
  ...
  SUBROUTINE INIT_QUEUES(NQUEUES)
    USE FIELD_ABORT_MODULE
    INTEGER(KIND=JPIM), INTENT(IN) :: NQUEUES

    IF ( NQUEUES < 2 ) THEN
      CALL FIELD_ABORT("NQUEUES MUST BE GREATER THAN 1 IN MULTI-QUEUE MODE")
    ENDIF

    FIELD_NQUEUES = NQUEUES
  END SUBROUTINE

  SUBROUTINE DELETE_QUEUES()
    FIELD_NQUEUES=1
  END SUBROUTINE
