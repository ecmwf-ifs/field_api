...
__all__ = ['ROCMAFAROpenMP']

class ROCMAFAROpenMP():
    """
    A class that defines the macros needed for GPU offload using AMD's flang based
    ...
    def host_data(cls, symbols):
        """
        Pragma to mark the start of a `target data use_device_addr` region.
        """

        return f"!$omp target data use_device_addr({','.join(symbols)})"
