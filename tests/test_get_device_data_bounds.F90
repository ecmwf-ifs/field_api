...
PROGRAM TEST_GET_DEVICE_DATA_BOUNDS
  ...
  CALL F_PTR%GET_DEVICE_DATA_FORCE(PTR_GPU, BLK_BOUNDS=[1,2])
  OKAY=.TRUE.
  ...
  CALL F_PTR%SYNC_HOST_FORCE(BLK_BOUNDS=[1,2])
  DO I=1,128
    DO J = 1,2
  ...
  CALL F_PTR%GET_DEVICE_DATA_FORCE(PTR_GPU2)

#ifdef OMPGPU
  !$omp target map(to:PTR_GPU2) map(tofrom:OKAY)
#else
