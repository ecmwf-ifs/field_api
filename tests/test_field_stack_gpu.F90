...
PROGRAM TEST_FIELD_STACK_GPU
    ...
    UBOUNDS(2) = NTOTAL
    UBOUNDS(3) = NBLKS
    CALL FIELD_NEW(F_BUFFER, DATA=BUFFER, LSTACK=.TRUE., MEMBER_LBOUNDS=MEMBER_LBOUNDS, MEMBER_MAP=MEMBER_MAP)
    ...
    CALL GET_STACK_MEMBER(F_BUFFER, 1, F_1DF)
    CALL GET_STACK_MEMBER(F_BUFFER, 2, F_3DF)
    CALL GET_STACK_MEMBER(F_BUFFER, 3, F_2DF)
    ...
    !...This should have triggered data movement on the full field stack
    !...thus every field should have status NDEVFRESH = 1
    IF (F_BUFFER%ISTATUS /= 1) CALL FIELD_ABORT("F_BUFFER STATUS SHOULD BE NDEVFRESH")
    IF (F_1DF%ISTATUS /= 1) CALL FIELD_ABORT("F_1DF STATUS SHOULD BE NDEVFRESH")
    IF (F_3DF%ISTATUS /= 1) CALL FIELD_ABORT("F_3DF STATUS SHOULD BE NDEVFRESH")
    IF (F_2DF%ISTATUS /= 1) CALL FIELD_ABORT("F_2DF STATUS SHOULD BE NDEVFRESH")
    ...
    !...This should update the status of the buffer and all stack members
    !...thus every field should have status NHSTFRESH = 2
    IF (F_BUFFER%ISTATUS /= 2) CALL FIELD_ABORT("F_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (F_1DF%ISTATUS /= 2) CALL FIELD_ABORT("F_1DF STATUS SHOULD BE NHSTFRESH")
    IF (F_3DF%ISTATUS /= 2) CALL FIELD_ABORT("F_3DF STATUS SHOULD BE NHSTFRESH")
    IF (F_2DF%ISTATUS /= 2) CALL FIELD_ABORT("F_2DF STATUS SHOULD BE NHSTFRESH")
    ...
    !...This should have triggered data movement on the full field stack
    !...thus every field should have status NDEVFRESH = 1
    IF (F_BLOCKED_BUFFER%ISTATUS /= 1) CALL FIELD_ABORT("F_BLOCKED_BUFFER STATUS SHOULD BE NDEVFRESH")
    IF (F_2DB%ISTATUS /= 1) CALL FIELD_ABORT("F_2DB STATUS SHOULD BE NDEVFRESH")
    IF (F_4DB%ISTATUS /= 1) CALL FIELD_ABORT("F_4DB STATUS SHOULD BE NDEVFRESH")
    IF (F_3DB%ISTATUS /= 1) CALL FIELD_ABORT("F_3DB STATUS SHOULD BE NDEVFRESH")
    ...
    !...This should update the status of the buffer and all stack members
    !...thus every field should have status NHSTFRESH = 2
    IF (F_BLOCKED_BUFFER%ISTATUS /= 2) CALL FIELD_ABORT("F_BLOCKED_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (F_2DB%ISTATUS /= 2) CALL FIELD_ABORT("F_2DB STATUS SHOULD BE NHSTFRESH")
    IF (F_4DB%ISTATUS /= 2) CALL FIELD_ABORT("F_4DB STATUS SHOULD BE NHSTFRESH")
    IF (F_3DB%ISTATUS /= 2) CALL FIELD_ABORT("F_3DB STATUS SHOULD BE NHSTFRESH")
