...
PROGRAM GPU_TO_CPU_INIT_VALUE
        ...
        IF(OKAY .EQV. .FALSE.)THEN
          CALL FIELD_ABORT ("ERROR WRONG VALUE ON DEVICE")
        ENDIF
        ...
        IF(O%STATS%TRANSFER_GPU_TO_CPU /= 1)THEN
          CALL FIELD_ABORT ("ERROR WRONG NUMBER OF TRANSFERS")
        ENDIF

        DO I=1,5
          DO J=1,5
            IF (PTR_HOST(I,J) /= 3)THEN
              CALL FIELD_ABORT ("ERROR WRONG VALUE ON HOST")
            ENDIF
          ENDDO
        ENDDO

        CALL FIELD_DELETE(O)
END PROGRAM GPU_TO_CPU_INIT_VALUE
