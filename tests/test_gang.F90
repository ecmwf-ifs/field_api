...
CALL GETENV ('GET_DEBUG_PRINT_LOCATION', CLENV); GET_DEBUG_PRINT_LOCATION = CLENV /= '0' .AND. CLENV /= ''
...
CALL FIELD_NEW (YLF4, CHILDREN=YLF3L, LBOUNDS=ILB, PERSISTENT=LLPERSISTENT, DATA=ZDATA4)
...
!FIELD_ABORT_FUNC => NULL()!TEST_GANG_ABORT
...
DO JFLD = 1, NDIM
  IF (YLF4%GET_STATUS () /= YLF3L(JFLD)%PTR%GET_STATUS ()) THEN
    PRINT *, JFLD
    WRITE (*, '("YLF4 = ",Z16.16)') YLF4%GET_STATUS ()
    WRITE (*, '("YLF3L (",I0,")=",Z16.16)') JFLD, YLF3L(JFLD)%PTR%GET_STATUS ()
