...
PROGRAM INIT_OWNER_DELAYED_GPU
        !TEST IF ACCESSING THE DEVPTR IS WORKING AFTER HAVING ACCESSED FIRST THE HOST PTR
        ...
        ASSOCIATE(UBOUNDS=>O%UBOUNDS)
#ifdef OMPGPU
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO MAP(TO:PTR,UBOUNDS)
#else
        !$ACC KERNELS PRESENT(PTR) COPYIN(UBOUNDS)
#endif
        DO I=10,21
          DO J=1,UBOUNDS(2)
            PTR(I,J)=42
