...
PROGRAM GATHER_SCATTER_COLLAPSE2_LASTDIM
!TEST THAT FIELD_GATHSCAT_COLLAPSE2 ONLY MODIFY VALUES THAT HAVE BEEN FILTERED
USE FIELD_MODULE
...
INTEGER, PARAMETER :: NPROMA = 10, NFLEVG = 5, NFLD = 2, JBLKA = 2, JBLKB = 5, NGPBLKS = JBLKB - JBLKA + 1

LOGICAL :: TRIG (NPROMA, 0:NFLEVG, JBLKA:JBLKB)

INTEGER :: I, J, JLEV, JPASS, JBOFF
...
DO JPASS = 1, 3
  ...
  SELECT CASE (JPASS)
    CASE (1)
      DO I =1,NPROMA
        IF (MOD (I,2) == 0) THEN !ONLY MODIFY EVEN COLUMNS
          TRIG (I, :, :)=.TRUE.
        ENDIF
      ENDDO
    CASE (2)
  ...
  CALL FIELD_NEW (FTRIG, DATA=TRIG, LBOUNDS=[1,0,JBLKA])
  
  !CREATE THE FIELD TO BE FILTERED BY GATHSCAT
  ALLOCATE (D (NPROMA, 0:NFLEVG, NFLD, JBLKL:JBLKU))
  
  D = 1
  
  CALL FIELD_NEW (FD, DATA=D, LBOUNDS=[1,0,1,JBLKL])
  ...
  IF (CLTYPE (JPASS) /= 'NULL') THEN
    ...
    IF (LBOUND (FILTERED_D, 2) /= 1) CALL FIELD_ABORT ('LBOUND MISMATCH')
    IF (UBOUND (FILTERED_D, 2) /= NFLD) CALL FIELD_ABORT ('UBOUND MISMATCH')
  ENDIF
